...
const HEALTH = {
  ...
  'hn-fb':  { url: AW_EXEC+'/fb-autopost/executions', method:'POST',
    ...
    headers:{'Content-Type':'application/json','X-Appwrite-Project':AW_PROJECT} },
  'hn-aw':  { url:'https://fra.cloud.appwrite.io/v1/health', method:'GET', headers:{}, mode:'no-cors' },
};
async function runHealthCheck(){
  await Promise.allSettled(Object.entries(HEALTH).map(async ([id, cfg])=>{
    ...
    try{
      const r = await fetch(cfg.url,{method:cfg.method,headers:cfg.headers,body:cfg.body||undefined,mode:cfg.mode||'cors',signal:AbortSignal.timeout(8000)});
